...
output: pdf_document
---
<!-- TODO: Add specific section on Markdown, how to format, best practice and so on. -->
<!-- TODO: Add information about the scope of pull requests so it is clear that a pull request is only allowed to contain changes to a single RFC. -->
<!-- TODO: Add a section that informs users on the best-practice use of glossary terms. Terms are to be used in "bold" at the first usage -->
...
+ RFCs can be proposed by either the RFC Editorial Committee, RFC authors appointed by the Time Machine Organization or any other interested public author.
*Note*: This document doesn't differentiate between individual and groups of authors when talking about *RFC authors*.
...
The official drafting phase starts with the merging of the initial RFC draft by the RFC Team and extends over a period of two weeks. The main RFC Markdown document as well as accompanying files, for example images, are stored in the `files/drafts/[RFC-id]` folder for the duration of the drafting phase.
...
Merged pull requests will trigger a `GitHub Action`[^github_action] that will compile the current version of all open draft documents and attach them as a ZIP file accessible on the workflow job details[^github_job_details]. These preview files are available for 90 days.
...
Yes, you will need a GitHub account if you want to directly contribute to the text, discuss issues or comment on pull requests then you will need a GitHub account. If you want to comment on a review candidate on OpenReview.net you will need an OpenReview.net. Both serve to make attributions of content and comments to individual persons possible and help to ensure scientific standards for the drafting and review process.

## Question: I have an idea for an RFC. What do I do?

Please contact the RFC Editorial Committee with your idea. They will help you proceed in creating an initial draft version of the idea if it is feasible.

## Question: I want to fix some errors in an RFC draft. How do I do that?

The easiest way is for you to sign into your GitHub account, create a fork of the main RFC repository into your own account, fix the error in your version of the document, commit it into your repository and open a pull request outlining your changes to the main RFC repository. It will then be visible, can be reviewed and eventually merged into the main draft document if the changes are accepted by the reviewers.

## Question: I have worked on a draft document myself and want to preview how the final PDF document would look like. How can I do this?

If you follow the official way of contributing by forking the main RFC repository anything you push to the `master` branch in your own repository will be automatically converted to a PDF document by triggering a GitHub action. You can access the ZIP file with the compiled draft documents in the GitHub job details page in the action section.

## Question: How do I see what drafts are currently available to contribute to or release candidates to review?
