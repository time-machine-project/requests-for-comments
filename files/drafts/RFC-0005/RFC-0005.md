...
The first reslease should permit to following components, essentiall meant for introducing the basic structure of the LTM infrastructure functioning. At this stage, the LTM Webspace essentily consists in a redocumentation of the existing. 
...
The **Project Editor** includes basic tools for perfoming internal operations 
...
###  Phase II : First Tools and Search Engine

In the Phase II the first tools/components are introduced. 

(HERE TALK ABOUT THE PROCEDURAL APPROACH AND GENERIC MODEL OF TOOLS/COMPONENT)

1. IIIF Source (Import an existing IIIF source). An imported source is immedialy searchable in the Search Engine. 
...
3. Image Annotator: Permit to annotate zone of images in IIIF collection by defining zones and associate them with Time Machine or Wikidata ID). This makes these elements immediatable searchage. 
4. Transcription tools: Permit to input structure data (from based) to the Data Graph. For each document a Data Mask is defined. 
5. Georeferencing tool : Permit to georefence a map part of the IIIF source
6. Photoalignment tools : Permits to connect a photo with a map and with other photos based on homologous points,. 


The annotated document are directly searchable in the first version of the Search Engine and the first version of the Node Pages are introducted for People and Places. 

(HERE TALK ABOUT NODE PAGES)

(FIRST TOOL MAYBE A SIMPLE COMMON LIBRARY USABLE AS JUPYTER NOTEBOOK, The first version of the Time Machine Langauge)
